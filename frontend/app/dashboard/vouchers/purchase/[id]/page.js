...
export default function PurchaseVoucherViewPage({ params }) {
  ...
  useEffect(() => {
    ...
    api.purchases.get(activeCompany.id, id)
  ...
  usePageShortcuts([
    ...
    { combo: "ctrl+shift+p", display: "Ctrl Shift P", label: "PDF", action: () => downloadPdf() },
  ], [voucher]);

  async function updateStatus(status) {
    try { const updated = await api.purchases.updateStatus(activeCompany.id, id, status); setVoucher((v) => ({...v, status: updated.status})); toast.success("Status updated."); }
  ...
  async function downloadPdf() {
    ...
    setDownloading(true);
    try {
      await downloadResponse(api.pdf.voucher({ kind: "Purchase Voucher", number: voucher.voucher_number, date: voucher.voucher_date, company: activeCompany, party: voucher.supplier, items: voucher.items||[], subtotal: voucher.subtotal, gstAmount: voucher.gst_amount, total: voucher.total, notes: voucher.notes }), `${voucher.voucher_number}.pdf`);
    } catch (err) { toast.error(err.message); }
  ...
  return (
    <div className="space-y-6 fade-in max-w-3xl">
      <div className="flex items-center justify-between no-print">
        <button onClick={() => router.push("/dashboard/vouchers/purchase")} className="flex items-center gap-1.5 text-sm text-paper-faint hover:text-paper"><ArrowLeft size={14}/> Back</button>
      ...
      <div className="bg-white text-[#16213e] rounded-xl p-8 sm:p-10 shadow-2xl shadow-black/40 print:shadow-none print:rounded-none">
        <div className="flex items-start justify-between border-b border-[#16213e]/15 pb-6 mb-6">
          <div>
            <p className="text-[10px] uppercase tracking-[0.18em] text-[#16213e]/50 font-mono mb-1">Purchase Voucher</p>
            <h1 className="font-display text-2xl font-bold">{activeCompany?.name}</h1>
            <p className="text-xs text-[#16213e]/60 mt-1">{activeCompany?.address}</p>
            {activeCompany?.gst_number && <p className="text-xs text-[#16213e]/60 font-mono">GSTIN {activeCompany.gst_number}</p>}
          </div>
          <div className="text-right">
            <p className="font-mono text-sm font-semibold">{voucher.voucher_number}</p>
        ...
        <div className="mb-8">
          <p className="text-[10px] uppercase tracking-[0.1em] text-[#16213e]/50 font-mono mb-1.5">Purchased from</p>
          <p className="font-semibold">{voucher.supplier?.name}</p>
          <p className="text-xs text-[#16213e]/60 mt-0.5">{voucher.supplier?.address}</p>
          <p className="text-xs text-[#16213e]/60">{voucher.supplier?.mobile}</p>
          {voucher.supplier?.gstin && <p className="text-xs text-[#16213e]/60 font-mono">GSTIN {voucher.supplier.gstin}</p>}
        </div>
          ...
          <tbody>
            {items.map((i) => (
              <tr key={i.id} className="border-b border-[#16213e]/10">
                <td className="py-2.5">{i.item_name}</td><td className="py-2.5 text-right num">{formatNumber(i.qty)}</td>
                <td className="py-2.5 text-right num">{formatCurrency(i.rate)}</td><td className="py-2.5 text-right num">{i.gst_percent}%</td>
        ...
        {voucher.notes && <div className="mt-8 pt-4 border-t border-[#16213e]/15"><p className="text-[10px] uppercase text-[#16213e]/50 font-mono mb-1">Notes</p><p className="text-sm text-[#16213e]/70">{voucher.notes}</p></div>}
